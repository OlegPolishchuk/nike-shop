...
import { LongOffset } from '../constants/map';
import { LatLon, PlaceMarkPosition } from '../types/types';
import { getBalloonPosition, getMapContainerCenter } from '../utils/utils';
...
const MapContainer = ({ center, stores }: Props) => {
  ...
  const handlePlaceMarkClick = (store: Store) => {
    ...
    map?.panTo([store.coordinates.latitude, store.coordinates.longitude + LongOffset], {
      flying: true,
    });
      ...
      >
        {stores.map((store) => (
          <Placemark
            ...
            options={{ iconColor: activePlacemarkId === store.id ? '#1d4ed8' : 'black' }}
